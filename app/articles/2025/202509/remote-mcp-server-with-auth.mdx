---
title: 管理画面のMCPサーバー化 ~認証認可の深掘り~
ogImageTitle: 管理画面のMCPサーバー化 ~認証認可の深掘り~
...
sequenceDiagram
    ...
    rect rgb(240, 240, 240)
      ...
      Server->>Server: Set proxyState encoding the original MCP Client information in state<br/> proxyState = base64({original_redirect_uri, original_state})
      Server->>Browser: HTTP 302 Redirect
      Browser->>Cognito: Authorization request<br/>(redirect_uri=MCP Server/oauth2/callback, state=proxyState)
    end

    Note over Cognito: User authorizes
    Cognito->>Browser: Redirect to callback with authorization code
    rect rgb(240, 240, 240)
      Browser->>Server: Authorization code callback with proxyState
      Server->>Server: Decode proxyState to restore the original MCP Client information<br/>(original_redirect_uri, original_state)
...
- リクエスト中のMCPクライアントが発行したstateとredirect_uriをエンコードしたものをstateに格納しておく
...
先ほどの認可エンドポイントでエンコードしたstateをデコードすると、元々のMCPクライアントのstateとredirect_uriを取得できるので、これを元にMCPクライアントにリダイレクトさせます。
...
app.post("/oauth2/token", async (c) => {
  ...
  const cognitoRequestBody: CognitoRequestBody =
...
一見、この処理はproxyする必要がないようにも思いますが、OAuth 2.0ではtokenリクエストでのリダイレクトURIは認可リクエストのものと同じにする必要があります。

ちなみに、OAuth 2.1ではtokenリクエストでのリダイレクトURIは不要となります。
...
ここまで見てきたように、認可リクエストとtokenリクエストもproxyさせることで、
DCRがサポートされていない中でもあらゆるMCPクライアントに対してリダイレクトURIを固定することができました。
...
ここまでで無事疎通が行えるようになりましたが、MCP仕様書に含まれていて登場していない仕様としてResource Indicatorsがあります。
...
得られたaccess tokenをデコードしてみると`aud`フィールドに、指定したリソースサーバーのURLが格納されています。
...
ただし、今回はリフレッシュトークンによるトークンの更新までは確認できていません。
同内容のissueが立っているので、もしかしたらclaude codeはまだ対応できていないのかもしれません。

<EmbedLink url="https://github.com/anthropics/claude-code/issues/5706" />

この辺は過渡期ということもあり、認可サーバー同様、MCPクライアント側も対応状況がまちまちですね...
