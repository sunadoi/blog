...
今回はキャッシュ機構を変更した結果、パフォーマンスがかなり安定したのでその方法を備忘録として記載しておきます。
...
sequenceDiagram
    ...
    participant Cache as キャッシュサーバー
...
sequenceDiagram
    ...
    Note over Client,Cache: リクエスト時の処理
    Client->>Server: リクエスト
    Server->>Cache: データ取得
    Cache-->>Server: データ返却
    Server-->>Client: レスポンス

    Note over Server,DB: 非同期での定期実行 (リクエストとは無関係)
    loop 一定期間ごと
        Server->>DB: 全データ取得
        DB-->>Server: データ返却
        Server->>Cache: データ格納
...
太平洋かってくらい穏やかですね
1万RPSくらいあるのにも関わらず、RDBのクエリ数は数十/sec程度で済んでいます
クエリ発行数が99%程度削減できました
